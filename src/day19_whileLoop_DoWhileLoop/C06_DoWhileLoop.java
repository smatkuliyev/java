...
*/
	...
	private static boolean kucukHarfKontrol(String sifre) {
		boolean khk = false;
		String harfler = "abcdefghÝjklmnoprstuvyzqwx";
		for (int i = 0; i < sifre.length(); i++) {
			if (harfler.contains(sifre.substring(i, i+1))) {
				khk = true;
				break;			//sonucu bulunca kodu calistirmayi birakmak icin
			}
		}
		// bu satira geldiginde sifrede kucuk harf varsa khk=true, hic kucuk harf yoksa khk=false
		if(!khk) {
